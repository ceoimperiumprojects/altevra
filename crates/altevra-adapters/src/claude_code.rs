...
impl ClaudeCodeAdapter {
    ...
    fn settings_json_content() -> String {
        ...
        //
        // P0 hardening (PLAN-ALIVE.md): bake the absolute --db path resolved
        // on the installing machine so a hook can never resolve a CWD-relative
        // shadow DB, and derive project/working_dir from $CLAUDE_PROJECT_DIR /
        // cwd inside hook-handle instead of the never-set $ALTEVRA_PROJECT.
        let db = altevra_core::paths::default_db_path().display().to_string();
        let cmd = |event: &str| format!("altevra hook-handle {event} --tool claude-code --db {db}");
